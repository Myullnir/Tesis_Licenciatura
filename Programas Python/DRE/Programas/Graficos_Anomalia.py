...
for REDES in ["Erdos-Renyi","RandomRegulars","Barabasi"]:
    ...
    if Subdividido == True:
        ...
        CarpCheck=[[root,dirs,files] for root,dirs,files in os.walk("../Anomalia/{}".format(REDES))]
        ...
        CarpCheck=[[root,files] for root,dirs,files in os.walk("../Anomalia/{}".format(REDES))]
...
#                            Opi0 = np.array([float(x) for x in Datos[1][1::]])
                    ...
                    plt.annotate(r"$\alpha=${}".format(ALFA), xy=(0.8,0.85),xycoords='axes fraction',fontsize=20,bbox=dict(facecolor='White', alpha=0.7))
