...
for REDES in ["Erdos-Renyi","RandomRegulars","Barabasi"]:
    ...
    for AGENTES in [1000]:
        ...
        for GM,igm in zip(Conjunto_Gm,np.arange(len(Conjunto_Gm))):
            ...
            DF = pd.read_csv("Grafico Fases Red={}_N=1000_Gm=_{}.csv".format(REDES,GM),delimiter = ",",header = None)
...
#            np.savetxt("Grafico Fases Red={}_N={}_GM=_{}.csv".format(REDES,AGENTES,GM), ZZ, delimiter = ",")
            ...
            for CDELTA,icdelta in zip(Conjunto_Cdelta,np.arange(len(Conjunto_Cdelta))):
                ...
                for ALFA,filas,columnas in zip(Conjunto_Alfa[2:27],Filas,Columnas):
...
#                        if nombre.split("_")[1] == "Opiniones":
                ...
                # -------------------------------------------------------------------------------------------------
                    ...
                    if ZZ[ZZ.shape[0]-1-Conjunto_Alfa.index(ALFA),icdelta] == 0:
                        color = "green"
                    elif ZZ[ZZ.shape[0]-1-Conjunto_Alfa.index(ALFA),icdelta] == 1:
                        color = "blue"
                    elif ZZ[ZZ.shape[0]-1-Conjunto_Alfa.index(ALFA),icdelta] == 2:
                        color = "red"
                    elif ZZ[ZZ.shape[0]-1-Conjunto_Alfa.index(ALFA),icdelta] == 3:
                        color = "purple"
