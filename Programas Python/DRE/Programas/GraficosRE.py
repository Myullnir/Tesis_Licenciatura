...
for REDES in ["Erdos-Renyi"]: #,"RandomRegulars","Barabasi"]:
    ...
    for AGENTES in [1000]:
        ...
        for GM,igm in zip(Conjunto_Gm[2::],np.arange(len(Conjunto_Gm))[2::]):
            ...
            for ALFA,ialfa in zip(Conjunto_Alfa,np.arange(len(Conjunto_Alfa))):
                ...
                for CDELTA,icdelta in zip(Conjunto_Cdelta,np.arange(len(Conjunto_Cdelta))):
                    if ALFA in [0.0,0.5,1] and CDELTA in [-0.5,0,0.5]:
...
#                            Opi0 = np.array([float(x) for x in Datos[1][1::]])
                        ...
                        plt.hist2d(Opiniones2D[0::2],Opiniones2D[1::2], density=True,bins=(79,79),cmap=plt.cm.Reds)
    #                    if REDES == "Barabasi":
    ...
    #                    plt.annotate(r"$\alpha$={},cos($\delta$)={},<k>={}".format(ALFA,CDELTA,GM), xy=(0.6,0.85),xycoords='axes fraction',fontsize=20,bbox=dict(facecolor='White', alpha=0.7))
                        ...
                        plt.ylabel(r"$x^2$")
                        plt.colorbar()
                        plt.savefig("../../../Imagenes/Redes Estáticas/Oporto/{}/GM={}/Hist2D_opiniones_alfa={:.3f}_Cdelta={:.2f}_N={}.png".format(REDES,GM,ALFA,CDELTA,AGENTES),bbox_inches = "tight")
    ...
    #                    plt.annotate(r"$\alpha$={},cos($\delta$)={},N={}".format(ALFA,CDELTA,AGENTES), xy=(0.6,0.85),xycoords='axes fraction',fontsize=20,bbox=dict(facecolor='White', alpha=0.7))
                        plt.savefig("../../../Imagenes/Redes Estáticas/Oporto/{}/GM={}/Grafico_opiniones_alfa={:.3f}_Cdelta={:.2f}_N={}.png".format(REDES,GM,ALFA,CDELTA,AGENTES),bbox_inches = "tight")
...
#                    Definitivamente son despreciables, dale para adelante.
            ...
            plt.ylim(min(Conjunto_Alfa),max(Conjunto_Alfa))
...
#            plt.annotate("Red={}, Gm={}".format(REDES,GM), xy=(0.6,0.85),xycoords='axes fraction',fontsize=20,bbox=dict(facecolor='White', alpha=0.7))
        ...
        #     plt.show()
            ...
            for CDELTA,icdelta in zip(Conjunto_Cdelta,np.arange(len(Conjunto_Cdelta))):
                ...
                for ALFA,filas,columnas in zip(Conjunto_Alfa[0:25],Filas,Columnas):
