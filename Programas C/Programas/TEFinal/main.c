...
int main(int argc, char *argv[]){
	...
	// Creo mis punteros a structs y los malloqueo.
		...
		int i_rearmar = 30;
		// Esto lo comento porque como voy a guardar las opiniones de todos los agentes, no tiene sentido guardar testigos.
		// int i_testigos = 5;
		// int i_momento = 0;
		// int i_gradosumado = 0;
		...
		// El vector de Activados no lo voy a usar, pero me da miedo sacarlo.
		ps_red->pd_Km = (double*) malloc((2+i_rearmar)*sizeof(double)); // Vector que guarda los grados medios de las redes armadas.
		
		
		// Voy a abrir cuatro archivos. Uno para anotar la evolución de opiniones.
		// El segundo es para guardar los datos de las actividades.
		// El tercero es para guardar las matrices de Adyacencias
		// El cuarto es para levantar los datos de la Tabla_Valores_TANH
		// Este archivo es el que guarda las opiniones del sistema
		char s_archivo1[355];
		sprintf(s_archivo1,"../Programas Python/DilAct/TEFinal/Evolucion_Sistema_alfa=%.3f_Cdelta=%.2f_N=%d_Iter=%d",ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,i_iteracion);
		FILE *pa_archivo1=fopen(s_archivo1,"w"); // Con esto abro mi archivo y dirijo el puntero a él.
		
		// Este archivo es el que guarda las actividades de los agentes y la Variación Promedio.
		char s_archivo2[355];
		sprintf(s_archivo2,"../Programas Python/DilAct/TEFinal/Datos_Variacion_alfa=%.3f_Cdelta=%.2f_N=%d_Iter=%d",ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,i_iteracion);
		FILE *pa_archivo2=fopen(s_archivo2,"w"); // Con esto abro mi archivo y dirijo el puntero a él.
		
		// Este archivo es el que guarda las matrices de Adyacencia construidas.
		char s_archivo3[355];
		sprintf(s_archivo3,"../Programas Python/DilAct/TEFinal/Datos_Adyacencia_alfa=%.3f_Cdelta=%.2f_N=%d_Iter=%d",ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,i_iteracion);
	...
	for(register int i_ciclos=0; i_ciclos<i_rearmar; i_ciclos++){
		...
		for(register int i_contador=0; i_contador<i_renovar_Adyacencia; i_contador++){
			...
			fprintf(pa_archivo2, "\t%lf",ps_red->d_Varprom);
		}
	}
	// for(register int i_sujeto=0; i_sujeto<i_testigos; i_sujeto++) for(register int i_opinion=0; i_opinion<2; i_opinion++){
			// *(ap_Testigos[i_sujeto]+i_momento*2+i_opinion+2) = ps_red->pd_Opi[a_Testigos[i_sujeto]*ps_datos->i_T+i_opinion+2];
