...
int main(int argc, char *argv[]){
	...
	time(&tt_prin);
	srand(time(NULL)); // Voy a definir la semilla a partir de un número entero que es el número de iteración  srand(time(NULL));
	...
	// Creo mis punteros a structs y los malloqueo.
		...
		ps_datos->f_alfa = strtof(argv[2],NULL)/100; // Controversialidad de los tópicos. Arranquemos con controversialidad intermedia. Voy a estar dividiendo esto acá para poder pasar enteros desde el instanciar.
		...
		ps_datos->i_Itextra = 100; // Este valor es la cantidad de iteraciones extra que el sistema tiene que hacer para cersiorarse que el estado alcanzado efectivamente es estable
		...
		char s_archivo1[255];
		sprintf(s_archivo1,"Datos_Opiniones_alfa=%.3f_Cdelta=%.3f_N=%d_Iter=%d",ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,i_iteracion);
