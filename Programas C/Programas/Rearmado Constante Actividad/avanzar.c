...
double Din1(ps_Red ps_var, ps_Param ps_par, ps_Tab ps_tab){
	...
	d_resultado = IntrpTANH(ps_par->f_alfa*d_opiniones_superpuestas,ps_tab); // Esto es lo que está dentro de la sumatoria en la ecuación dinámica.
...
double Din2(ps_Red ps_var, ps_Param ps_par, ps_Tab ps_tab){
	...
	for(ps_var->i_agente2=0; ps_var->i_agente2<ps_par->i_N; ps_var->i_agente2++){
		if(ps_var->pi_Ady[ps_var->i_agente*ps_var->pi_Ady[1]+ps_var->i_agente2+2] == 1) d_sumatoria += Din1(ps_var,ps_par,ps_tab);
		else d_sumatoria += 0;
...
int Iteracion(ps_Red ps_var, ps_Param ps_par, ps_Tab ps_tab, double (*pf_Dinamica)(ps_Red ps_var, ps_Param ps_par, ps_Tab ps_tab) ){
	...
	for(ps_var->i_agente=0; ps_var->i_agente<ps_par->i_N; ps_var->i_agente++){
		...
		for(ps_var->i_topico=0; ps_var->i_topico<ps_par->i_T; ps_var->i_topico++){
			...
			ps_var->pd_Opi[ps_var->i_agente*ps_par->i_T+ps_var->i_topico+2] = RK4(ps_var->pd_PreOpi, ps_var, ps_par, ps_tab,pf_Dinamica);
		}
	}
	return 0;
}
