...
int main(int argc, char *argv[]){
	...
	// Creo mis punteros a structs y los malloqueo.
		...
		ps_datos->i_pasosprevios = 50; // Elegimos 20 de manera arbitraria con Pablo y Sebas. Sería la cantidad de pasos hacia atrás que miro para comparar cuanto varió el sistema
		...
		if(strcmp("Barabasi",argv[2])==0) sprintf(s_archivo1,"../Programas Python/DRE/RevisionNCC/Barabasi/GM=%d/Datos_Opiniones_alfa=%.2f_Cdelta=%.2f_N=%d_Gm=%d_ID=%d_Iter=%d",ps_datos->i_Gradomedio,ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,ps_datos->i_Gradomedio,ps_datos->i_ID,i_iteracion);
		if(strcmp("ErdosRenyi",argv[2])==0) sprintf(s_archivo1,"../Programas Python/DRE/RevisionNCC/Erdos-Renyi/GM=%d/Datos_Opiniones_alfa=%.2f_Cdelta=%.2f_N=%d_Gm=%d_ID=%d_Iter=%d",ps_datos->i_Gradomedio,ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,ps_datos->i_Gradomedio,ps_datos->i_ID,i_iteracion);
		if(strcmp("RandomR",argv[2])==0) sprintf(s_archivo1,"../Programas Python/DRE/RevisionNCC/RandomRegulars/GM=%d/Datos_Opiniones_alfa=%.2f_Cdelta=%.2f_N=%d_Gm=%d_ID=%d_Iter=%d",ps_datos->i_Gradomedio,ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,ps_datos->i_Gradomedio,ps_datos->i_ID,i_iteracion);
