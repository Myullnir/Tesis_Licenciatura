...
int main(int argc, char *argv[]){
	...
	// Creo mis punteros a structs y los malloqueo.
		...
		ps_datos->f_alfa = strtof(argv[2],NULL)/200; // Controversialidad de los tópicos. Arranquemos con controversialidad intermedia. Voy a estar dividiendo esto acá para poder pasar enteros desde el instanciar.
		...
		char s_archivo1[355];
		sprintf(s_archivo1,"../Programas Python/DilAct/DinaReg/Datos_Opiniones_alfa=%.3f_Cdelta=%.2f_N=%d_Iter=%d",ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,i_iteracion);
		...
		// char s_archivo2[355];
		// sprintf(s_archivo2,"../Programas Python/DilAct/ActividadConexa/Datos_Testigos_alfa=%.3f_Cdelta=%.2f_N=%d_Iter=%d",ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,i_iteracion);
		...
		char s_archivo2[355];
		sprintf(s_archivo2,"../Programas Python/DilAct/DinaReg/Evolucion_Sistema_alfa=%.3f_Cdelta=%.2f_N=%d_Iter=%d",ps_datos->f_alfa,ps_datos->f_Cosangulo,ps_datos->i_N,i_iteracion);
		FILE *pa_archivo2=fopen(s_archivo2,"w"); // Con esto abro mi archivo y dirijo el puntero a él.
	...
	// fprintf(pa_archivo2,"\tOpiniones de los testigos %d %d %d %d %d\n",a_Testigos[0],a_Testigos[1],a_Testigos[2],a_Testigos[3],a_Testigos[4]);
	...
	while(i_contador < ps_datos->i_Itextra){
		...
		do{
			...
			for(register int i_j=0; i_j<ps_datos->i_N*ps_datos->i_T; i_j++) ps_red->pd_PreOpi[i_j+2] = ps_red->pd_Opi[i_j+2];
			// for(register int i_sujeto=0; i_sujeto<i_testigos; i_sujeto++) for(register int i_opinion=0; i_opinion<2; i_opinion++) fprintf(pa_archivo2, "\t%lf",ps_red->pd_Opi[a_Testigos[i_sujeto]*ps_datos->i_T+i_opinion+2]);
			// fprintf(pa_archivo2, "\n");
			Iteracion(ps_red,ps_datos,ps_tab,pf_EcDin);
			Escribir_d(ps_red->pd_Opi,pa_archivo2);
			i_IndiceOpiPasado++;
	...
	fprintf(pa_archivo1, "\tGrado de los agentes\n");
	for(register int i_i=0; i_i<ps_datos->i_N; i_i++){
		i_gradosumado = 0;
		for(register int i_j=0; i_j<ps_datos->i_N; i_j++) i_gradosumado += ps_red->pi_Ady[i_i*ps_datos->i_N+i_j+2];
		fprintf(pa_archivo1, "\t%d",i_gradosumado);
	}
	fprintf(pa_archivo1, "\n");
	fprintf(pa_archivo1,"\tActividad de los agentes\n");
	Escribir_d(ps_red->pd_Act,pa_archivo1); // Vector de Actividad
