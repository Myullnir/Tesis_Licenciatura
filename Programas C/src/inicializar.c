...
int Visualizar(struct Red var, int i_Crit){
	if(i_Crit==0){ //En este caso observo los vectores de opinión
		...
		for(int i_i=0; i_i<i_Filas; i_i++){
			for(int i_j=0; i_j<i_Columnas; i_j++) printf("%.2lf\t",var.pd_Opi[i_i*i_Columnas+i_j+2]);
			printf("\n");
		}
	}
		...
		for(int i_i=0; i_i<i_Filas; i_i++){
			for(int i_j=0; i_j<i_Columnas; i_j++) printf("%.2lf\t",var.pd_Ang[i_i*i_Columnas+i_j+2]);
			printf("\n");
		}
	}
